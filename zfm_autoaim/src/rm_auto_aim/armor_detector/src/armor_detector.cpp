  ...
  if (!armors_.empty() && classifier != nullptr) {
      ...
      std::for_each(
        std::execution::par, armors_.begin(), armors_.end(), [this, &input](Armor &armor) {
          ...
          // 5. Do classification (internally mutex-protected)
          classifier->classify(input, armor);
          // 6. Correct the corners of the armor (serialized cv::PCA)
...
std::vector<Light> Detector::findLights(const cv::Mat &rgb_img,
  ...
  for (const auto &contour : contours) {
    ...
    if (isLight(light)) {
      ...
      int sampled = (n + step - 1) / step;
      int diff = std::abs(sum_r - sum_b);
      if (diff * 1.0f / sampled > light_params.color_diff_thresh) {
