...
ArmorDetectorNode::ArmorDetectorNode(const rclcpp::NodeOptions &options)
  ...
  ZFM_REGISTER_LOGGER("armor_detector", "~/zfm2026-log", DEBUG);
  ZFM_INFO("armor_detector", "Starting ArmorDetectorNode!");
...
void ArmorDetectorNode::imageCallback(
  ...
  try {
    auto odom_to_gimbal = tf2_buffer_->lookupTransform(
        odom_frame_, "gimbal_link", tf2::TimePointZero);
    ...
    ZFM_ERROR("armor_detector", "Something Wrong when lookUpTransform");
    return;
  }
...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector() {
  ...
  int detect_color_int = declare_parameter("detect_color", 1);
  EnemyColor detect_color =
      (detect_color_int == 0) ? EnemyColor::RED : EnemyColor::BLUE;
  ...
  ZFM_ASSERT_MSG(fs::exists(model_path) && fs::exists(label_path),
...
std::vector<Armor> ArmorDetectorNode::detectArmors(
  ...
  // Calculate FPS
  frame_count_++;
  double elapsed = (final_time - last_fps_time_).seconds();
  if (elapsed >= 30.0) {
    fps_ = frame_count_ / elapsed;
    frame_count_ = 0;
    last_fps_time_ = final_time;
    ZFM_INFO("armor_detector", "FPS: {:.1f}", fps_);
  }
...
RCLCPP_COMPONENTS_REGISTER_NODE(zfm::auto_aim::ArmorDetectorNode)
