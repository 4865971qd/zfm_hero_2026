...
public:
  ...
  : Node("camera_driver", options), frame_cnt_(0) {
    ZFM_REGISTER_LOGGER("camera_driver", "~/zfm2026-log", INFO);
    ZFM_INFO("camera_driver", "Starting VideoPlayerNode!");
    // Get parameters
    ...
    if (!std::filesystem::exists(video_file)) {
      ZFM_ERROR("camera_driver", "Video file {} does not exist!", video_file.string());
      rclcpp::shutdown();
      return;
    }
    cap_.open(video_path);
    if (!cap_.isOpened()) {
      ZFM_ERROR("camera_driver", "Video file open failed!");
      rclcpp::shutdown();
      return;
    }
    ...
    if (camera_info_manager_->validateURL(camera_info_url)) {
      ...
      ZFM_WARN("camera_driver", "Invalid camera info URL: {}", camera_info_url);
    }
    ...
    // Publish at the specified frame rate
    int period_ms = std::max(1, 1000 / frame_rate);
    timer_ = this->create_wall_timer(std::chrono::milliseconds(period_ms), [this]() {
      cap_ >> frame_;
      if (frame_.empty()) {
        ZFM_INFO("camera_driver", "Video file ends!");
        if (!is_loop_) {
          rclcpp::shutdown();
          return;
        } else {
      ...
      if (frame_cnt_ < start_frame_) {
        ZFM_INFO("camera_driver", "Skipping frame {}", frame_cnt_);
        return;
      }
...
RCLCPP_COMPONENTS_REGISTER_NODE(zfm::camera_driver::VideoPlayerNode)
