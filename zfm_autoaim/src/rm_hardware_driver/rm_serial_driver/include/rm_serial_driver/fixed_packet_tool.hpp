...
bool FixedPacketTool<capacity>::simpleSendPacket(const FixedPacket<capacity> &packet) {
  if (transporter_->write(packet.buffer(), capacity) == capacity) {
    ...
    // reconnect
    ZFM_ERROR("serial_driver", "transporter_->write() failed");
    transporter_->close();
    transporter_->open();
    return false;
  }
...
bool FixedPacketTool<capacity>::recvPacket(FixedPacket<capacity> &packet) {
  ...
  if (recv_len > 0) {
    ...
    if (checkPacket(tmp_buffer_, recv_len)) {
      ...
      ZFM_INFO("serial_driver", "checkPacket() failed, check if it is a broken frame");
    ...
    ZFM_ERROR("serial_driver", "transporter_->read() failed");
    // reconnect
    transporter_->close();
    transporter_->open();
    // 串口错误
