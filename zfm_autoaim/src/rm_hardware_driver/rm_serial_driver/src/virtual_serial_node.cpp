...
// Copyright (C) ZFM Vision Group. All rights reserved.
...
public:
  explicit VirtualSerialNode(const rclcpp::NodeOptions &options) : Node("serial_driver", options) {
    ZFM_REGISTER_LOGGER("serial_driver", "~/zfm-log", INFO);
    ZFM_INFO("serial_driver", "Starting VirtualSerialNode!");
    ...
    serial_receive_data_msg_.mode_flag = 0;
    ...
    // Heartbeat
    heartbeat_ = zfm::HeartBeatPublisher::create(this);
    ...
    timer_ = this->create_wall_timer(std::chrono::milliseconds(5), [this]() {
      ...
      Eigen::Vector3d rpy  = zfm::utils::getRPY(q_eigen.toRotationMatrix());
      q.setRPY(rpy[0], 0, 0);
      transform_stamped_.transform.rotation = tf2::toMsg(q);
      transform_stamped_.header.frame_id = "odom";
  ...
  void setMode(SetModeClient &client, const uint8_t mode) {
    ...
    while (!client.ptr->wait_for_service(1s)) {
      ...
      ZFM_INFO("serial_driver", "service {} not available, waiting again...", service_name);
    }
    if (!client.ptr->service_is_ready()) {
      ZFM_WARN("serial_driver", "Service: {} is not available!", service_name);
      return;
    }
    // Send request
...
private:
  zfm::HeartBeatPublisher::SharedPtr heartbeat_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(zfm::serial_driver::VirtualSerialNode)
