...
int Solver::selectBestArmor(const std::vector<Eigen::Vector3d> &positions,
    ...
    debug_info_.selected_idx = final_sel;
    debug_info_.raw_selected_idx = raw_sel;
    ...
    debug_info_.selected_dist =
        final_sel >= 0 && final_sel < static_cast<int>(positions.size()) ?
        positions[final_sel].norm() : -1.0;
    debug_info_.best_dist = raw_dist_valid ? raw_dist : -1.0;
    debug_info_.second_dist = std::isfinite(second_dist) && second_dist < DBL_MAX ?
        second_dist : -1.0;
...
GimbalCommand Solver::solve(const Target &target, double current_time) {
    ...
    debug_info_.fly_time = flying_time;
    ...
    debug_info_.aim_yaw_deg = yaw * 180.0 / M_PI;
    debug_info_.aim_pitch_deg = pitch * 180.0 / M_PI;
    debug_info_.cmd_yaw_deg = cmd.yaw;
    debug_info_.cmd_pitch_deg = cmd.pitch;
    debug_info_.fire = cmd.fire_advice;
    ...
    if (selected_prediction_valid) {
        ...
        debug_info_.selected_idx = selected_visualization_index;
        debug_info_.selected_dist = selected_visualization_position.norm();
    } else {
