...
void Tracker::update(const std::list<Armor> &armors, std::chrono::steady_clock::time_point t) {
    ...
    if (!armors.empty()) {
        ...
        if (min_pos_diff < effective_max_match_dist &&
            ...
            Eigen::Vector4d predicted_measurement(
                target_state_(0) - target_state_(8) * std::cos(target_state_(6)),
                target_state_(2) - target_state_(8) * std::sin(target_state_(6)),
                target_state_(4) + target_state_(9),
                target_state_(6));
            Eigen::Vector4d residual = measurement_ - predicted_measurement;
            ...
            debug_info_.prediction = predicted_measurement;
            debug_info_.residual = residual;
