...
int main(int argc, char *argv[]) {
    ...
    while (!exiter.exit()) {
        ...
        if (do_plot && tracker.hasTarget()) {
            auto target = tracker.getTarget();
            const auto &track_dbg = tracker.debugInfo();
            const auto &solve_dbg = solver.debugInfo();
            ...
            const double serial_age_ms = (serial.isOpen() && has_rx) ?
                std::chrono::duration<double>(t - serial.lastRxTime()).count() * 1000.0 :
                -1.0;
            plotter.plot(fmt::format(
                ...
                imu_sync.bracket_span_ms,
                serial_age_ms,
                track_dbg.matched ? 1 : 0,
                track_dbg.measurement[0],
                track_dbg.measurement[1],
                track_dbg.measurement[2],
                obs_yaw_deg,
                target.state[0],
                target.state[2],
                target.state[4],
