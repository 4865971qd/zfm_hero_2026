  ...
  explicit HikCameraNode(const rclcpp::NodeOptions & options) : Node("hik_camera", options)
  {
    ...
    if (device_index >= static_cast<int>(device_list.nDeviceNum)) {
      RCLCPP_WARN(this->get_logger(),
        ...
        device_index, device_list.nDeviceNum);
      device_index = 0;
    }
    MV_CC_CreateHandle(&camera_handle_, device_list.pDeviceInfo[device_index]);
...
private:
  ...
  void declareParameters()  {
    ...
    double exposure_time = this->declare_parameter("exposure_time", 12000.0, param_desc);
